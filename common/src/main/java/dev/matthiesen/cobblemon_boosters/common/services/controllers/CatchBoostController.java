...
public final class CatchBoostController implements IBoostController<CatchBoost> {
    ...
    public void queueResponseHandler(CommandContext<CommandSourceStack> ctx) {
        Util.handleQueueResponse(ctx, getBoostQueue(), BoostersConfig.getCatchMessages());
    }

    @Override
    public void queueClearHandler(CommandContext<CommandSourceStack> ctx) {
        Util.handleQueueClear(ctx, getBoostQueue(), BoostersConfig.getCatchMessages().boostQueueCleared());
    }

    @Override
    public SupportedBoosterTypes getType() {
        return SupportedBoosterTypes.CATCH;
    }
    ...
    public void hydrateFromCache() {
        CatchBoost cachedActive = BoostersConfig.getActiveCatchBoost();
        setActiveBoost(cachedActive != null && cachedActive.getTimeRemaining() > 0 ? cachedActive : null);

        this.queue.clear();
        var cachedQueue = BoostersConfig.getQueuedCatchBoosts();
        if (cachedQueue != null) {
